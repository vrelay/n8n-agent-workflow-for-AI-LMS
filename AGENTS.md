...
rg -n "LMS:" n8n/packages/frontend/editor-ui   # all our patches so far
```

### Already customized (as of LMS work)

Search for `LMS:` markers. Currently includes:

- `DefaultLayout.vue` — main sidebar commented out
- `WorkflowLayout.vue` — sidebar + AI overlays commented out
- `WorkflowsView.vue` — overview header/tabs/insights/callouts/onboarding empty/template recs commented out; page is workflows list + search/sort/filters
...
- Students use a **small set of nodes**; we constrain UX, not invent a new engine.
- Prefer hiding chrome (sidebar, tabs, callouts, AI overlays) over rewriting business logic.
